<template>
  ...
  >
    ...
    <VTab v-if="userId"><VIcon icon="tabler-lock"/>Security</VTab>
  </VTabs>
  ...
  <VCardText>
    <VWindow v-model="currentTab">
      ...
      <VWindowItem :value="1" v-if="userId">
        <!-- Use the UserSettingsSecurity component -->
        <UserSettingsSecurity :uid="userId" />
      </VWindowItem>
    </VWindow>
  </VCardText>
</template>
...
const userId = ref<string | null>(null); // Initially, no uid is known

// Handler for the userUpdated event
function handleUserUpdated(newUid: string) {
  userId.value = newUid; // Update uid to show the Security tab
